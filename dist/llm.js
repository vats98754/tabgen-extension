...
export async function planTabs(input) {
    // Try Hugging Face first; if unavailable, fallback
    const [token, model] = await Promise.all([
        getSync(KEY_HF_TOKEN),
        getSync(KEY_HF_MODEL),
    ]);
    if (token) {
        try {
            const resp = await callHuggingFaceInference(token, model || DEFAULT_MODEL, input);
            if (resp)
                return resp;
        }
        catch (e) {
...
async function callHuggingFaceInference(token, model, input) {
...
function validColor(c) {
    ...
    return allowed.includes(c) ? c : 'blue';
}
